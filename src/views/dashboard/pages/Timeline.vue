<template>
  <v-container id="timeline" fluid tag="section">
    <base-v-component heading="Timelines" link="components/timelines" />

    <v-row>
      <v-col>
        <v-timeline align-top>
          <v-timeline-item v-for="(timeline, i) in timelines" :key="i" :color="timeline.color" :icon="timeline.icon"
            fill-dot large>
            <v-card class="pa-6">
              ...
              <template v-if="timeline.action">
                ...
                <v-menu v-model="menu" bottom offset-y origin="top left" right transition="scale-transition">
                  <template v-slot:activator="{ attrs, on }">
                    <v-btn v-bind="attrs" :color="timeline.action" large rounded v-on="on">
